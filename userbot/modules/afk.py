...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**! Mohon Maaf [{user.first_name}](tg://user?id={user.id}) Sedang AFK.**\
        \n**Karena :** `{string}`")
    else:
        await afk_e.edit(f"**! Mohon Maaf [{user.first_name}](tg://user?id={user.id}) Sedang AFK.**")
    if user.last_name:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**! Mohon Maaf [{user.first_name}](tg://user?id={user.id}) Sedang AFK Sejak** `{afk_since}` **Yang Lalu.**\
                            \n**Karena :** `{AFKREASON}`")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"**! Mohon Maaf [{user.first_name}](tg://user?id={user.id}) Sedang AFK Sejak** `{afk_since}` **Yang Lalu.**\
                            \n**Karena :** `{AFKREASON}`")
                else:
